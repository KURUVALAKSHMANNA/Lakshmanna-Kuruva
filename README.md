<h1 align="center">Hi 👋, I'm Kuruva Lakshmanna</h1>
<h3 align="center">Aspiring Java Developer | Software Developer | BTech CSE Graduate</h3>


I am a B.Tech Computer Science student passionate about building software applications using **Java, Spring Boot, MySQL, React, and Web Technologies**. I enjoy learning backend development, solving coding problems, and creating real-world projects.

---

## 👨‍💻 About Me

* 🎓 B.Tech in Computer Science and Engineering
* 💻 Interested in Java Full Stack Development
* 🌱 Currently learning DSA, APIs, Spring Boot, and React
* 🔧 Strong interest in backend systems and database-driven applications
* 🎯 Goal: To become a skilled Software Engineer

---

## 🛠️ Skills

### Programming Languages

* Java
* SQL
* Python

### Web Technologies

* HTML
* CSS
* JavaScript
* React

### Backend & Database

* Spring Boot
* JDBC
* MySQL
* REST APIs

### Tools

* Eclipse IDE
* VS Code
* MySQL Workbench
* Postman
* Git & GitHub

---

## 🚀 Projects

### Movie Collection App

A responsive movie search web application using the OMDb API.

**Features:**

* User Login and Sign Up
* Movie search functionality
* Displays movie poster, title, year, and type
* Responsive movie card layout

### Bank Management System

A Java-based banking application using JDBC and MySQL.

**Features:**

* Create account
* Deposit and withdraw amount
* Fund transfer
* Transaction history
* Database connectivity using JDBC

### Hospital Management System

A Java console-based project for managing hospital records.

**Features:**

* Patient management
* Doctor management
* Appointment handling
* Simple and clean Java logic

### Portfolio Website

A personal portfolio website to showcase my skills, projects, certifications, and contact details.

---

## 📜 Certifications

* Java Full Stack Certification
* Full Stack Java Internship
* Python Programming Internship
* Web Development Certification
* Introduction to AI & ML
* Animation with JavaScript and jQuery

---
## 📊 GitHub Stats

![Lakshmanna's GitHub Stats](https://github-readme-stats.vercel.app/api?username=KURUVALAKSHMANNA&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=KURUVALAKSHMANNA&layout=compact&theme=tokyonight)


---

## 🔥 GitHub Streak

![GitHub Streak](https://streak-stats.demolab.com?user=KURUVALAKSHMANNA&theme=tokyonight)

---

## 📫 Connect with Me

* LinkedIn: https://www.linkedin.com/in/lakshmanna-kuruva-749250334/
* GitHub: https://github.com/KURUVALAKSHMANNA
* Email: [kuruvalakshmanna4154@gmail.com](mailto:kuruvalakshmanna4154@gmail.com)

---

## ✨ Quote

> Consistency is the key to success.

---

⭐ Thank you for visiting my profile!
